    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(data_bn)):
            features = feature_extractor(
                [i for i in data['img']], return_tensors='pt').to(device)
            ...
            for i in range(n_beit_layers):
                ...
                #apply layernorm
                if i == n_beit_layers-1:
                    n_l = beit.layernorm(l_output)
                else:
                    n_l = beit.beit.encoder.layer[i+ 1].layernorm_before(l_output)

                layer_output.append(torch.mean(n_l, 1))
...
for i in tqdm(range(n_beit_layers)):
    model.fit(train_outputs[:, i, :].cpu().detach(
    ).numpy().reshape(-1, emb_dim), train_labels)
    pred = model.predict(test_outputs[:, i, :].cpu(
    ).detach().numpy().reshape(-1, emb_dim))
